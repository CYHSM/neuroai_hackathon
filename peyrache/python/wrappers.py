...
def makePositionsExtended(path, file_order, episodes, n_ttl_channels = 1, optitrack_ch = None, names = ['ry', 'rx', 'rz', 'x', 'y', 'z'], update_wake_epoch = True, rotations = 2):
        """
        ...
        # pos ry [0, 2pi]       
        # start unwrap
        for coord in ["ry", "rx", "rz"]:
            #init_hd = position[[coord]].data[0]
            
            # ugly bodge code - feel free to numpy this

            init_hd = position[coord].iloc[0]
            pos_tmp = list(position[coord].iloc[:].values)
            running_total = [init_hd] 

            for i in range(len(pos_tmp) - 1):

                diff = pos_tmp[i+1] - pos_tmp[i]
                if pos_tmp[i+1]  - pos_tmp[i] > np.pi:
                    diff = -1 * (2*np.pi - pos_tmp[i+1] + pos_tmp[i])

                elif pos_tmp[i+1]  - pos_tmp[i] < -1*np.pi:
                    diff =  (pos_tmp[i+1] + 2*np.pi -  pos_tmp[i])

                prev_last_total = running_total[-1]
                new_total = diff + prev_last_total

                if prev_last_total > 2*rotations*np.pi:
                    new_total = new_total - 2*rotations*np.pi
                elif new_total < 0:
                    new_total = 2*rotations*np.pi + new_total

                running_total.append(new_total)
            position[coord] = running_total
...
def loadPosition(path, events = None, episodes = None, n_ttl_channels = 1, optitrack_ch = None, names = ['ry', 'rx', 'rz', 'x', 'y', 'z'], update_wake_epoch = True, rotations=1):
        """
        ...
        if rotations > 1:
            print("using extended range")
            makePositionsExtended(path, events, episodes, n_ttl_channels, optitrack_ch, names, update_wake_epoch)
        else:
