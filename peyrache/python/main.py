...
Running the script repeatedly will result in different results
This script is clever and caches results - I am however less clever and broke this
remove the "Analysis" folder that gets created in raw_data before running again
...
Adapted from: https://github.com/PeyracheLab/StarterPack
Original doc string below
...
#position = loadPosition(data_directory, events, episodes, n_ttl_channels = 2, optitrack_ch = 1, rotations=1)
...
# plot some new tuning curves with some funky 4pi radian ranges
# need to adjust the position['ry'] - this is the head direction of the rodent
from functions import computeExtendedTuningCurves
from functions import smoothExtendedTuningCurves


position = loadPosition(data_directory, events, episodes, n_ttl_channels = 2, optitrack_ch = 1, rotations=2)

tuning_curves = computeExtendedTuningCurves(spikes, position['ry'], wake_ep, 60)
tuning_curves = smoothExtendedTuningCurves(tuning_curves, 10, 2)
for i, n in enumerate(tuning_curves.columns):
	subplot(5,5,i+1, projection = 'rectilinear')
	plot(tuning_curves[n])	
show()
