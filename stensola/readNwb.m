...
function readNwb(filename)
    ...
    for i = 1:numUnits
        ...
        unitData = h5read(filename, unitPath);
        fprintf('Unit %u, number of spikes: %u\n', i, numel(unitData));
%         if i == 1
            % make a path plot of one unit
            figure, plot(posx, posy);
            hold on;
            spkPos = mapSpikes2Positions(unitData, post, pos);
            plot(spkPos(:, 2), spkPos(:, 3), 'Marker', '.', 'Color', [1 0 0], 'MarkerSize', 10, 'LineStyle', 'none');
            title(sprintf('Path plot (spikes on the path) for cell %s', unitList{i}));
            xlabel(posUnit);
            ylabel(posUnit);
            hold off;
%         end
